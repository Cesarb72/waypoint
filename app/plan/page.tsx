...
import { upsertPlan, type StoredStop, type StoredPlan, loadPlanById } from '@/lib/planStorage';
...
export default function PlanPage() {
  ...
  // - or new plan with waypoint name as title + stop 1 label
  useEffect(() => {
    ...
    if (urlPlanId) {
      ...
      if (stored) {
        ...
        setPlan({
          ...
          notes: stored.notes ?? '',
          location: (stored.location ?? waypointLocation ?? waypointName) || '',
          stops:
            stored.stops && stored.stops.length > 0
              ...
              : [createStop(waypointName || 'Main stop')],
        });
        setInitialized(true);
        return;
      }
    ...
    setPlan({
      ...
      attendees: '',
      notes: '',
      location: waypointLocation || waypointName || '',
      stops: [createStop(waypointName || 'Main stop')],
    });
    setInitialized(true);
  }, [initialized, urlPlanId, waypointLocation, waypointName]);
  ...
  // ✅ Derived validation flags (safe here because plan is not null)
  ...
  function buildStoredInputFromDraft(draft: PlanDraft): {
    ...
    return {
      ...
      notes: draft.notes || '',
      stops: storedStops,
      location: draft.location || waypointLocation || waypointName || undefined,
    };
  }
  ...
  const effectiveLocation = plan.location || waypointLocation || waypointName || '';
        ...
        <div className="mx-auto max-w-2xl px-4 py-8 space-y-6">
          ...
          {hasTriedSubmit && !isCoreInfoValid && (
            <div className="rounded-lg border border-amber-500/70 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
              ...
              <span className="font-semibold">time</span>. These are required to save or
              share.
            </div>
          )}

          {/* Map preview (static for now, but location-aware) */}
          {effectiveLocation && (
            <section className="space-y-2 rounded-xl border border-slate-800 bg-slate-900/70 p-4">
              ...
              <p className="text-xs text-slate-400">
                Location:{' '}
                <span className="text-slate-200 font-medium">{effectiveLocation}</span>
              </p>
              <div className="mt-2 h-36 w-full rounded-lg border border-slate-800 bg-gradient-to-br from-slate-900 to-slate-800 flex items-center justify-center text-[11px] text-slate-500">
                Static map preview placeholder for this area. (V2: live tiles here.)
              </div>
            </section>
          )}
...
<p className="text-xs text-slate-500">
  You can reorder stops to match how the night flows.
</p>
      ...
      {/* 👇 Make the tiny date/time picker icons white so they stand out on dark bg */}
      <style jsx global>{`
